...
// *EXREMELY IMPORTANT CONCEPT - VARIALBE DECLARATION w/ TYPE *

// Back to variables as there is a part of these which we have not yet discussed, each variable
// that you use in the C programming language MUST be decalred prior to the assignment of any value
// to that variable, it can be assigned combinatorally on instantiation but it must be declared first.

// This is precisely why when we look at the example code in the second example where variables are used
// the authors have had to define 1) that a variable will be used, 2) what the name of that variable will
// be and 3) what TYPE of data that variable will hold. All three conditions must be met in the C programming
// language before data can be assigned to a variable which is the crux of a Strongly Typed programming
// language. The compiler is given notice that it must make available the requisite memory for the TYPE
// of data being stored along with the amount of data that TYPE will be expected to consume in its lifecycle.
...
// variables in this manner. The rest of the book and exercises will open you up to the rest of these
